  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        if (!res.ok) throw new Error(`Could not load ${GROUPS_URL} (HTTP ${res.status})`);

        const data = (await res.json()) as IndustryGroups;
        if (cancelled) return;

        // sanitize: normalize symbols + dedupe
        const cleaned: IndustryGroups = {};
        for (const [industry, syms] of Object.entries(data || {})) {
        ...
        if (!cancelled) setErr(e?.message ?? "Failed to load industry groups");
      } finally {
