...
function getAlertDirection(alert: RawAlert): "UP" | "DOWN" | "OTHER" {
  const raw = String(
    alert.direction_rule_direction || alert.direction || alert.signal || ""
  ).toUpperCase();
...
export default function CommandCenter({ onNavigate }: CommandCenterProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <section
        className="
          relative overflow-hidden rounded-[32px] p-6 md:p-7 transition-colors
          border border-slate-200 bg-white text-slate-900 shadow-[0_18px_55px_rgba(15,23,42,0.10)]
          dark:border-white/15 dark:bg-[radial-gradient(circle_at_top_left,rgba(59,130,246,0.16),transparent_34%),radial-gradient(circle_at_top_right,rgba(16,185,129,0.12),transparent_30%),linear-gradient(135deg,#101827_0%,#162335_55%,#0b111c_100%)] dark:text-white dark:shadow-[0_22px_70px_rgba(0,0,0,0.32)]
        "
      >
        <div className="pointer-events-none absolute inset-0 opacity-0 dark:opacity-[0.18] [background-image:linear-gradient(rgba(255,255,255,0.06)_1px,transparent_1px),linear-gradient(90deg,rgba(255,255,255,0.06)_1px,transparent_1px)] [background-size:38px_38px]" />
        ...
        <div className="relative z-10">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <p className="mt-2 max-w-2xl text-sm md:text-base text-slate-600 dark:text-slate-300">
                Live signal monitoring, alert mood, open trade status, and system health in one trading workspace.
              </p>
            </div>

            <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs text-slate-700 backdrop-blur dark:border-white/10 dark:bg-white/[0.04] dark:text-slate-300">
              <div className="flex items-center gap-2">
                <ShieldCheck
                  className={
                    connOk
                      ? "h-4 w-4 text-emerald-500 dark:text-emerald-300"
                      : "h-4 w-4 text-rose-500 dark:text-rose-300"
                  }
                />
          ...
          {err && (
            <div className="mt-5 rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-xs text-rose-700 dark:border-rose-300/20 dark:bg-rose-500/10 dark:text-rose-100">
          ...
          <div className="mt-7 grid gap-4 md:grid-cols-4">
            <div
              className={`flex min-h-[170px] flex-col justify-between rounded-3xl border px-5 py-5 shadow-sm dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] ${alertMood.cardClass}`}
            >
            ...
            <div className="flex min-h-[170px] flex-col justify-between rounded-3xl border border-orange-200 bg-orange-50 px-5 py-5 text-slate-900 shadow-sm dark:border-orange-300/25 dark:bg-[radial-gradient(circle_at_top_right,rgba(251,146,60,0.13),transparent_34%),#151f2e] dark:text-slate-100 dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.08)]">
                ...
                <span
                  className={
                    ...
                    (last24hAlerts.length === 0
                      ...
                      : upCount > downCount
                      ? "border-emerald-200 bg-emerald-100 text-emerald-700 dark:border-emerald-200/25 dark:bg-emerald-300/15 dark:text-emerald-100"
                      : downCount > upCount
                      ? "border-orange-200 bg-orange-100 text-orange-700 dark:border-orange-200/25 dark:bg-orange-300/15 dark:text-orange-100"
                      : "border-blue-200 bg-blue-100 text-blue-700 dark:border-blue-200/25 dark:bg-blue-300/15 dark:text-blue-100")
                  }
                >
                  {last24hAlerts.length === 0
                    ? "QUIET"
              ...
              <div>
                ...
                <div className="mt-1 text-[11px] text-slate-600 leading-snug dark:text-slate-300">
                  {recentAlertDetail}
                </div>
              </div>
      ...
      <section
        className="
          relative overflow-hidden rounded-[32px] p-6 md:p-7 transition-colors
          border border-slate-200 bg-white text-slate-900 shadow-[0_18px_55px_rgba(15,23,42,0.10)]
          dark:border-white/15 dark:bg-[linear-gradient(135deg,#101827_0%,#162335_55%,#0b111c_100%)] dark:text-white dark:shadow-[0_22px_70px_rgba(0,0,0,0.28)]
        "
      >
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-between gap-3">
            <div>
              <div className="text-[10px] font-bold uppercase tracking-[0.22em] text-slate-500 dark:text-slate-500">
                Activity stream
              </div>
            ...
            <div className="rounded-full border border-slate-200 bg-slate-50 px-3 py-1 text-[10px] font-semibold text-slate-600 dark:border-white/10 dark:bg-white/[0.04] dark:text-slate-300">
              {recentActivity.length} latest
            </div>
          </div>

          {recentActivity.length === 0 ? (
            <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-5 text-xs text-slate-500 dark:border-white/10 dark:bg-white/[0.04] dark:text-slate-400">
            ...
            <div className="mt-5 space-y-3">
              {recentActivity.map((item, idx) => (
                <div
                  key={`${item.time}-${idx}`}
                  className="flex items-start gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 shadow-sm dark:border-white/10 dark:bg-[#0b1423]/80 dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]"
                >
                  <div className="mt-1.5 h-2 w-2 rounded-full bg-slate-400 shadow-[0_0_14px_rgba(148,163,184,0.35)] dark:bg-slate-300 dark:shadow-[0_0_14px_rgba(148,163,184,0.45)]" />
...
function MetricCard({
  ...
  return (
    <div
      className={`flex min-h-[170px] flex-col justify-between rounded-3xl border px-5 py-5 text-slate-900 shadow-sm dark:text-slate-100 dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] ${styles.card}`}
    >
...
function MiniCount({
  ...
  const cls =
    tone === "up"
      ? "text-emerald-700 bg-emerald-100 border-emerald-200 dark:text-emerald-200 dark:bg-emerald-300/10 dark:border-emerald-200/20"
      : tone === "down"
      ? "text-orange-700 bg-orange-100 border-orange-200 dark:text-orange-200 dark:bg-orange-300/10 dark:border-orange-200/20"
      : "text-slate-700 bg-slate-100 border-slate-200 dark:text-slate-200 dark:bg-white/5 dark:border-white/10";
...
function QuickAction({
  ...
  return (
    <button
      onClick={onClick}
      className="group inline-flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-xs font-semibold text-slate-700 transition-all hover:-translate-y-[1px] hover:border-emerald-300 hover:bg-emerald-50 hover:text-emerald-700 dark:border-white/15 dark:bg-white/[0.04] dark:text-slate-200 dark:hover:border-emerald-300/35 dark:hover:bg-emerald-300/10 dark:hover:text-white"
    >
      {children}
      <ArrowRight className="h-3 w-3 text-slate-400 transition group-hover:text-emerald-600 dark:group-hover:text-emerald-200" />
    </button>
  );
}
