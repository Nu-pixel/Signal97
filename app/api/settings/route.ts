  ...
  try {
    ...
    if (!res.ok) {
      return NextResponse.json(
        { settings: null, ok: false, error: `VM API error: ${res.status} ${text}` },
        { status: 502 }
      );
    }

    const data = text ? JSON.parse(text) : { settings: null };
    return NextResponse.json(data);
  } catch (e: any) {
    return NextResponse.json(
      { settings: null, ok: false, error: `Cannot reach VM API (${e?.message ?? "unknown"})` },
      { status: 502 }
    );
  }
}
